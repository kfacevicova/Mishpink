...
wilcox_rbc(acc_overall$Desktop_mean, acc_overall$Smartphone_mean)
...
overall_test2 <- acc_overall2 %>%
  ...
  test = list(wilcox.test(Desktop_mean, Smartphone_mean,
                            paired = TRUE, exact = FALSE)),
  rbc = wilcox_rbc(Desktop_mean, Smartphone_mean),
  .groups = "drop"
  ) %>%
  mutate(
    V = sapply(test, \(x) unname(x$statistic)),
  ...
  select(task_type, n_pairs, median_diff, V, p_value, p_holm, r, rbc) %>%
  arrange(task_type)
