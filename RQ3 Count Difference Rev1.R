...
Count_diff$Order <- as.factor(Count_diff$Order)
...
order_results <- Count_diff %>%
  ...
  summarise(
    ...
    rbc = effectsize::rank_biserial(
      actions_diff ~ Order)$r_rank_biserial,
    .groups = "drop"
  ) %>%
  mutate(
  ...
  select(task, n, n_DS, n_SD, median_DS, median_SD, W, p_value, p_holm, rbc) %>%
  arrange(task)
...
order_overall <- Count_diff %>%
  ...
  summarise(mean_diff = mean(actions_diff, na.rm = TRUE),
              .groups = "drop")
...
effectsize::rank_biserial(
  order_overall$mean_diff ~ order_overall$Order)$r_rank_biserial
